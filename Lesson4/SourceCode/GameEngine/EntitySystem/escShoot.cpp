   ...
   ecs.system<InputState>()
        .each([&](InputState& state) {
            inputQuery.each([&](InputHandlerPtr& input) {
                state.shoot = input.ptr->GetInputState().test(eIC_Shoot);// разименование указателя с обращением к полю
            });
        });
    ...
    ecs.system<const Position, const BouncePlane, Bullet, RenderProxyPtr>()
        .each([&](flecs::entity e, const Position& pos, const BouncePlane& plane, Bullet& bullet, 
        ...
        if (bullet.started) {
            bullet.lTime -= e.delta_time();
        }
        if (bullet.lTime <= 0.0f) {
            e.destruct();
            delete proxy.ptr;
        }
            });
