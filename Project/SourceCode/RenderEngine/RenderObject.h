...
public:
	...
	virtual void SyncObjectWithProxy() = 0;

	void GetPosition(float* result) { m_pRenderProxy->GetPosition(result); }
	void GetScale(float* result) { m_pRenderProxy->GetScale(result); }
	void GetRotationAngle(float* result) { m_pRenderProxy->GetRotationAngle(result); }
